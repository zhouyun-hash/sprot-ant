<template>
  <view class="page">
    ...
    <template v-else>
      <!-- 雷达图：使用 echarts（echarts-for-uniapp 官方 dist 不含雷达，故采用 echarts + uni-echarts 组件） -->
...
async function loadStudentId() {
  const data = await request<{ id?: number }>({ url: '/students/me', method: 'GET' })
  const id = typeof data.id === 'number' ? data.id : Number(data.id)
  if (!Number.isFinite(id)) throw new Error('无法获取学生信息')
  studentId.value = id
}
...
async function loadReport() {
  ...
  const data = await request<ReportPayload>({
    url: `/report/student/${studentId.value}`,
    method: 'GET',
  })
...
async function loadScores() {
  ...
  try {
    const data = await request<{
      ...
      url: `/scores?studentId=${studentId.value}&page=1&pageSize=50`,
      method: 'GET',
    })
...
async function loadReportHistory() {
  ...
  try {
    const data = await request<{ items: ReportPayload[] }>({
      url: `/report/student/${studentId.value}/history?page=1&pageSize=50`,
      method: 'GET',
    })
